...
export const aiChatRouter = createTRPCRouter({
	...
	getAllDialogs: protectedProcedure
		.input(
			z.object({
				...
				status: z
					.enum(['all', 'active', 'completed', 'blocked'])
					.default('all'),
		...
		.query(async ({ ctx, input }) => {
			...
			const dialogsData = await Promise.all(
				messagesQuery.map(async (group) => {
					...
					const lastMessage =
						await ctx.prisma.aiChatMessage.findFirst({
					...
					return {
						...
						пользователь: {
							...
							аватар:
								user?.image ||
								`https://i.pravatar.cc/40?u=${group.userId}`,
						},
			...
			if (input.search) {
				...
				filteredDialogs = dialogsData.filter(
					(d) =>
						d.пользователь.имя
							.toLowerCase()
							.includes(searchLower) ||
						d.пользователь.email
							.toLowerCase()
							.includes(searchLower) ||
						d.последнееСообщение
							.toLowerCase()
							.includes(searchLower),
				);
			}
	...
	getDialogById: protectedProcedure
		...
		.query(async ({ ctx, input }) => {
			...
			const сообщения = messages.map((msg) => ({
				...
				обработано: true,
				токеныИспользовано:
					msg.isAI ? Math.floor(msg.content.length / 4) : undefined, // Примерная оценка токенов
				времяГенерации: msg.isAI ? 1.5 : undefined,
				модель: msg.modelName || undefined,
			}));

			// Статистика
			const userMessages = сообщения.filter(
				(m) => m.отправитель === 'user',
			);
			const aiMessages = сообщения.filter((m) => m.отправитель === 'ai');
			const totalTokens = aiMessages.reduce(
				(sum, m) => sum + (m.токеныИспользовано || 0),
				0,
			);
			...
			const диалог = {
				...
				пользователь: {
					...
					email: user.email || '',
					аватар:
						user.image || `https://i.pravatar.cc/40?u=${user.id}`,
				},
	...
	getStats: protectedProcedure
		...
		.query(async ({ ctx, input }) => {
			...
			// Активность по часам
			const hourlyActivity = await ctx.prisma.$queryRaw<
      ...
      `;
			...
			const активностьПоЧасам = Array.from({ length: 24 }, (_, час) => {
				const hourData = hourlyActivity.find(
					(h) => Number(h.hour) === час,
				);
				return {
					час,
			...
			const статистика = {
				...
				успешныхОтветов:
					aiMessages > 0 ?
						Math.round(
							(aiMessages / (userMessages + aiMessages)) *
								100 *
								10,
						) / 10
					:	0,
				...
				популярныеТемы: [
					{
						тема: 'проекты',
						количество: Math.floor(totalMessages * 0.3),
					},
					{
						тема: 'навыки',
						количество: Math.floor(totalMessages * 0.25),
					},
					{
						тема: 'технологии',
						количество: Math.floor(totalMessages * 0.2),
					},
					{
						тема: 'карьера',
						количество: Math.floor(totalMessages * 0.15),
					},
					{
						тема: 'контакты',
						количество: Math.floor(totalMessages * 0.1),
					},
				],
				активностьПоЧасам,
			};
	...
	testSettings: protectedProcedure
		.input(
			z.object({
				тестовоеСообщение: z
					.string()
					.default('Привет! Расскажи о проектах Кирилла.'),
				модель: z
					.enum([
						'llama3-8b-8192',
						'llama3-70b-8192',
		...
		.mutation(async ({ input }) => {
			console.log(
				'Тестирование настроек AI с сообщением:',
				input.тестовоеСообщение,
			);
	...
	sendAIResponse: protectedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			try {
				...
				throw new Error(
					`Ошибка обращения к AI: ${error instanceof Error ? error.message : 'Неизвестная ошибка'}`,
				);
			}
		}),
});
