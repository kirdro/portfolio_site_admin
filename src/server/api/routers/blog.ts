...
const BlogPostSchema = z.object({
  title: z.string().min(1, "Заголовок не может быть пустым").max(200, "Заголовок слишком длинный"),
  slug: z.string().min(1, "Слаг не может быть пустым").max(100, "Слаг слишком длинный"),
...
const BlogTagSchema = z.object({
  name: z.string().min(1, "Название тега не может быть пустым").max(50, "Название тега слишком длинное"),
  slug: z.string().min(1, "Слаг тега не может быть пустым").max(50, "Слаг тега слишком длинный"),
  color: z.string().regex(/^#[0-9A-F]{6}$/i, "Неверный формат цвета").default("#00FF99"),
});
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const post = await ctx.prisma.blogPost.create({
        data: {
          ...
          tags: {
            connect: input.tags.map(tagSlug => ({ slug: tagSlug })),
          },
        },
        include: {
          author: true,
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (input.data.tags) {
        updateData.tags = {
          ...
          connect: input.data.tags.map(tagSlug => ({ slug: tagSlug })),
        };
      }
  ...
  deleteTag: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (existingTag._count.posts > 0) {
        throw new Error(`Тег используется в ${existingTag._count.posts} постах. Сначала удалите тег из всех постов.`);
      }
