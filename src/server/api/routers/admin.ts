...
export const adminRouter = createTRPCRouter({
  ...
  users: createTRPCRouter({
    ...
    getActivity: protectedProcedure
      ...
      .query(async ({ input }) => {
        ...
        const [projectsCreated, messagesGeneral, messagesAi] = await Promise.all([
        ...
        return {
          summary: {
            ...
            messagesGeneral,
            messagesAi,
            totalActivity: projectsCreated + messagesGeneral + messagesAi,
          },
      ...
      .query(async () => {
        const [
          ...
          prisma.user.count({
            where: {
              OR: [
                { ChatMessage: { some: { createdAt: { gte: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000) } } } },
                { AiChatMessage: { some: { createdAt: { gte: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000) } } } },
                { Project: { some: { createdAt: { gte: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000) } } } },
              ]
            }
          }),
  ...
  projects: createTRPCRouter({
    ...
    update: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        // Проверяем права администратора
        if (ctx.session.user.role !== "ADMIN") {
          throw new Error("Недостаточно прав для редактирования проектов");
        }
  ...
  skills: createTRPCRouter({
    ...
    getAll: protectedProcedure
      .input(z.object({
        category: z.enum(["Frontend", "Backend", "DevOps", "Tools", "Other"]).optional(),
    ...
    update: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        // Проверяем права администратора
        if (ctx.session.user.role !== "ADMIN") {
          throw new Error("Недостаточно прав для редактирования навыков");
        }
    ...
    reorder: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        // Проверяем права администратора
        if (ctx.session.user.role !== "ADMIN") {
          throw new Error("Недостаточно прав для изменения порядка навыков");
        }
    ...
    getStats: protectedProcedure
      .query(async () => {
        ...
        const categoryStats = skillsByCategory.reduce((acc, item) => {
  ...
  getStats: protectedProcedure
    .query(async () => {
      ...
      const recentActivity = await Promise.all([
        ...
        prisma.aiChatMessage.count({ where: { createdAt: { gte: thirtyDaysAgo } } }),
      ]);
