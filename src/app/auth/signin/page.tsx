...
export default function SignInPage() {
  const [providers, setProviders] = useState<Record<string, ProviderType> | null>(null);
    ...
    <main className="min-h-screen flex items-center justify-center" 
      ...
      <div className="w-full max-w-md p-8 relative z-10">
        ...
        <div className="cyber-card p-6">
          <div className="mb-6">
            <h2 className="text-xl font-semibold mb-2" style={{color: '#00FF99'}}>
              АВТОРИЗАЦИЯ
            </h2>
            <p className="text-sm font-mono" style={{color: '#B8C5C0'}}>
          ...
          <form onSubmit={handleEmailSignIn} className="mb-6">
            <div className="mb-4">
              <label className="block text-sm font-mono mb-2" style={{color: '#E6F4EF'}}>
                EMAIL АДРЕС:
              </label>
              <input
                type="email"
          ...
          <div className="flex items-center mb-6">
            <div className="flex-1 h-px" style={{backgroundColor: '#1C2A25'}} />
            <span className="px-3 text-sm font-mono" style={{color: '#B8C5C0'}}>
              ИЛИ
            </span>
            <div className="flex-1 h-px" style={{backgroundColor: '#1C2A25'}} />
          </div>
          ...
          <div className="space-y-3">
            {providers && Object.values(providers).map((provider) => {
              ...
              return (
                ...
                >
                  {isLoading ? "[ПОДКЛЮЧЕНИЕ...]" : `[ВХОД ЧЕРЕЗ ${provider.name.toUpperCase()}]`}
                </button>
              );
            })}
          </div>
          ...
          <div className="mt-6 p-3 border border-line rounded" style={{backgroundColor: '#0F1214', borderColor: '#1C2A25'}}>
            <p className="text-xs font-mono text-center" style={{color: '#B8C5C0'}}>
              ⚠️ Все попытки входа логируются<br/>
