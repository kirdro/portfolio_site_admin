...
export default function ChatPage() {
  ...
  const [selectedMessage, setSelectedMessage] = useState<ChatMessageData | null>(null);

  // Получаем реальные данные из БД
  const { data: chatStats, isLoading: loadingStats } = api.chat.getStats.useQuery();
  const { data: generalMessages, isLoading: loadingGeneral } = api.chat.getMessages.useQuery({ 
    limit: 50, 
    type: "general" 
  });
  const { data: aiMessages, isLoading: loadingAi } = api.chat.getMessages.useQuery({ 
    limit: 50, 
    type: "ai" 
  });

  // Обработанные данные для совместимости с интерфейсом
  const mockStats: ChatStatsData = chatStats ? {
  ...
  const обработчикПереключенияВкладки = useCallback((tab: "general" | "ai" | "stats") => {
    setActiveTab(tab);
            ...
            <div className="flex items-center justify-between">
              <div>
                <div className="text-2xl font-bold text-cyan glyph-glow">
                  {loadingStats ? "..." : mockStats.messagesLast24h}
                </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                <div className="text-2xl font-bold text-orange-400 glyph-glow">
                  {loadingStats ? "..." : mockStats.moderationActions}
                </div>
