...
export default function UsersPage() {
	...
	>();
	const [selectedUser, setSelectedUser] = useState<UserTableData | null>(
		null,
	);
	...
	const обработчикПоиска = useCallback(
	...
	const обработчикФильтраРоли = useCallback(
		(role: 'USER' | 'ADMIN' | 'all') => {
			...
			<div className='bg-panel border border-line rounded-lg bevel p-4'>
				<div className='flex flex-col md:flex-row gap-4'>
					...
					<div className='flex-1'>
						<label
							htmlFor='search'
							className='block text-sm font-medium text-soft mb-2'
						>
							Поиск пользователей:
						</label>
						<input
