...
export function ChatMessages({ 
  ...
  const обработчикВыбораСообщения = useCallback((message: ChatMessageData) => {
    setSelectedMessageId(message.id);
    onMessageSelect(message);
  }, [onMessageSelect]);

  // Обработчик удаления сообщения
  const обработчикУдаленияСообщения = useCallback((messageId: string, e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  const обработчикБлокировкиПользователя = useCallback((userId: string, e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  const отфильтрованныеСообщения = messages.filter(message => {
    ...
    const matchesUser = filterUser === "" || message.userName === filterUser;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between text-sm text-soft mb-4">
        <div>
          Показано {отфильтрованныеСообщения.length} из {messages.length} сообщений
        </div>
        <div className="flex items-center space-x-2">
          <div className={`w-3 h-3 rounded-full ${type === "general" ? "bg-neon" : "bg-cyan"}`} />
          ...
          >
            <div className="flex items-start justify-between">
              ...
              <div className="flex items-start space-x-3 flex-1">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center space-x-2 mb-1">
                    ...
                    {message.isBlocked && (
                      <span className="px-2 py-1 bg-red-500/20 border border-red-500 
              ...
              <div className="flex items-center space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
                <button
                  onClick={(e) => обработчикУдаленияСообщения(message.id, e)}
                ...
                {!message.isBlocked && (
                  <button
                    onClick={(e) => обработчикБлокировкиПользователя(message.userId, e)}
      ...
      {отфильтрованныеСообщения.length > 10 && (
        <div className="flex justify-center mt-6">
          <div className="flex items-center space-x-2">
            <button className="px-3 py-1 bg-subtle border border-line rounded text-sm
