...
export function ChatStats({ stats, loading }: ChatStatsProps) {
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {Array.from({ length: 3 }).map((_, index) => (
            <div key={index} className="bg-subtle rounded-lg p-6 animate-pulse">
  ...
  const максЗначениеОбщий = Math.max(...stats.dailyActivity.map(d => d.generalMessages));
  const максЗначениеИИ = Math.max(...stats.dailyActivity.map(d => d.aiMessages));
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-subtle border border-line rounded-lg bevel p-6">
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-base font-bold">Сообщения за неделю</h3>
        ...
        <div className="bg-subtle border border-line rounded-lg bevel p-6">
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-base font-bold">Активные пользователи</h3>
            <div className="text-2xl">👥</div>
          </div>
        ...
        <div className="bg-subtle border border-line rounded-lg bevel p-6">
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-base font-bold">Модерационные действия</h3>
      ...
      <div className="bg-subtle border border-line rounded-lg bevel p-6">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-xl font-bold text-base">📊 Активность по дням</h3>
            ...
            <div key={day.date} className="space-y-2">
              <div className="flex items-center justify-between text-sm">
                ...
                <div className="flex items-center space-x-4 text-soft">
                  ...
                  <span>Всего: {day.generalMessages + day.aiMessages}</span>
                </div>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-subtle border border-line rounded-lg bevel p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-bold text-base">🏆 Самые активные пользователи</h3>
          </div>
          
          <div className="space-y-3">
            {stats.mostActiveUsers.map((user, index) => (
              <div key={user.userId} className="flex items-center justify-between p-3 bg-panel rounded border border-line">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-soft font-mono">ID: {user.userId}</div>
                  </div>
                </div>
                <div className="text-right">
                  ...
                  <div className="text-xs text-soft">сообщений</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="bg-subtle border border-line rounded-lg bevel p-6">
          <h3 className="text-lg font-bold text-base mb-4">📈 Детальная статистика</h3>
          
          <div className="space-y-4">
            <div className="flex items-center justify-between p-3 bg-panel rounded border border-line">
              <div className="flex items-center space-x-3">
                <div className="text-xl">💬</div>
                <span className="font-medium">Всего сообщений</span>
              </div>
            ...
            <div className="flex items-center justify-between p-3 bg-panel rounded border border-line">
              <div className="flex items-center space-x-3">
                <div className="text-xl">📊</div>
                <span className="font-medium">Среднее в день</span>
              </div>
            ...
            <div className="flex items-center justify-between p-3 bg-panel rounded border border-line">
              <div className="flex items-center space-x-3">
                <div className="text-xl">🎯</div>
                <span className="font-medium">Сообщений на пользователя</span>
              </div>
              <span className="text-xl font-bold text-purple-400 glyph-glow">
                {Math.round(stats.totalMessages / stats.activeUsers)}
              </span>
            </div>
            ...
            <div className="flex items-center justify-between p-3 bg-panel rounded border border-line">
              <div className="flex items-center space-x-3">
                ...
                <span className="font-medium">Модераций в день</span>
              </div>
