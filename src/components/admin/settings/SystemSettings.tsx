...
export function SystemSettings({
	...
	const обработчикИзменения = useCallback(
		(field: keyof SystemSettingsData, value: any) => {
	...
	if (false) {
		return (
			<div className='flex items-center justify-center py-12'>
				<div className='text-neon text-lg'>⏳ Загрузка настроек...</div>
			</div>
		);
	}
				...
				<div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
					<div>
						...
						<input
							...
							onChange={(e) =>
								обработчикИзменения('siteName', e.target.value)
							}
					...
					<div>
						...
						<select
							...
							onChange={(e) =>
								обработчикИзменения('language', e.target.value)
							}
							className='w-full px-4 py-2 bg-panel border border-line rounded text-base
                       focus:border-neon transition-colors'
						>
