...
export function MainPhotoSettings() {
	...
	const { data: settings, refetch: refetchSettings } =
		api.settings.getAll.useQuery() as any;
	...
	useEffect(() => {
		if (settings) {
			const mainPhotoSetting = settings.find(
				(setting: any) => setting.key === 'main_photo',
			);
			if (mainPhotoSetting && mainPhotoSetting.value) {
				try {
					const photoData =
						typeof mainPhotoSetting.value === 'string' ?
	...
	return (
		<div className='cyber-card p-6'>
			...
			<div className='mb-6'>
				<h3
					className='text-xl font-mono font-bold glyph-glow mb-2'
					style={{ color: '#00FF99' }}
				>
					ФОТО ВЛАДЕЛЬЦА
				</h3>
				<p
					...
					style={{ color: '#B8C5C0' }}
				>
					Загрузите фото для отображения на главной странице
					портфолио-сайта kirdro.ru
				</p>
			</div>
			...
			<div className='space-y-4'>
				...
				<div className='bg-bg-subtle border border-line rounded p-4 bevel'>
					<h4 className='font-mono font-bold mb-3 text-cyan'>
						РЕКОМЕНДАЦИИ:
					</h4>
					<div
					...
					>
						...
						<div className='flex items-center'>
							<span className='text-neon mr-2'>▸</span>
							Стиль: профессиональное фото в соответствии с
							киберпанк темой
						</div>
					</div>
				</div>
				...
				{currentPhotoUrl && (
					<div className='bg-bg-subtle border border-line rounded p-4 bevel'>
						<h4 className='font-mono font-bold mb-3 text-purple-400'>
						...
						<div className='flex items-center space-x-4 p-4 bg-bg border border-line rounded'>
							...
							<div>
								<div
									className='font-mono text-lg glyph-glow'
									style={{ color: '#00FF99' }}
								>
									Кирилл Дроздов
								</div>
