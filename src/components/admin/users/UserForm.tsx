...
export function UserForm({ user, onClose, onSave }: UserFormProps) {
  ...
  const подтвердитьДействие = useCallback(async () => {
    ...
    setActionType(null);
  }, [actionType, selectedRole, user, updateRoleMutation, toggleBlockMutation, onSave, onClose]);
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="flex-1 min-w-0">
              <h3 className="text-lg font-medium text-base">{user.name}</h3>
              <p className="text-sm text-soft">{user.email}</p>
              <div className="mt-2">
                <UserStatusBadge role={user.role} isBlocked={false} />
              </div>
            </div>
          </div>
          ...
          {активность && (
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-subtle p-4 rounded-lg border border-line">
                <div className="text-sm text-soft">Активность за 30 дней</div>
                <div className="mt-2 space-y-1">
                  <div className="flex justify-between">
                    <span className="text-xs text-soft">Проекты:</span>
                    <span className="text-xs text-neon font-mono glyph-glow">
                      {активность.summary.projectsCreated}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-xs text-soft">Сообщения:</span>
                    <span className="text-xs text-cyan font-mono">
                      {активность.summary.messagesGeneral + активность.summary.messagesAi}
                    </span>
                  </div>
                </div>
              </div>
              <div className="bg-subtle p-4 rounded-lg border border-line">
                <div className="text-sm text-soft">Общая статистика</div>
                <div className="mt-2 space-y-1">
                  <div className="flex justify-between">
                    <span className="text-xs text-soft">Всего проектов:</span>
                    <span className="text-xs text-neon font-mono glyph-glow">
                      {user._count?.projects || 0}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-xs text-soft">Всего сообщений:</span>
                    <span className="text-xs text-cyan font-mono">
                      {(user._count?.generalMessages || 0) + (user._count?.aiMessages || 0)}
                    </span>
                  </div>
                </div>
              </div>
          ...
          <div className="space-y-3">
            <label className="block text-sm font-medium text-base">Роль пользователя:</label>
          ...
          <div className="space-y-3">
            <label className="block text-sm font-medium text-base">Доступ к системе:</label>
            <button
        ...
        {showConfirmation && (
          <div className="bg-subtle border-t border-line p-6">
            <div className="text-sm text-soft mb-4">
              {actionType === "role" && (
                <>Вы уверены, что хотите изменить роль пользователя на "{selectedRole === "ADMIN" ? "Администратор" : "Пользователь"}"?</>
              )}
              {actionType === "block" && (
                <>Функция блокировки/разблокировки будет добавлена в следующих версиях</>
              )}
            </div>
            <div className="flex space-x-3">
              <button
                onClick={подтвердитьДействие}
                disabled={updateRoleMutation.isPending || toggleBlockMutation.isPending}
              ...
              >
                {(updateRoleMutation.isPending || toggleBlockMutation.isPending) ? "Сохранение..." : "Подтвердить"}
              </button>
              <button
