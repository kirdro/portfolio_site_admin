...
export function ContactsList({
	...
	const [filterStatus, setFilterStatus] = useState<
		ContactData['status'] | 'all'
	>('all');
	const [filterPriority, setFilterPriority] = useState<
		ContactData['priority'] | 'all'
	>('all');
	const [sortBy, setSortBy] = useState<'date' | 'priority'>('date');

	// Обработчик клика по обращению
	const обработчикКликаПоОбращению = useCallback(
		(contact: ContactData) => {
	...
	const обработчикИзмененияСтатуса = useCallback(
		(
			contactId: string,
	...
	const получитьИконкуСтатуса = useCallback(
		(status: ContactData['status']) => {
			switch (status) {
				case 'new':
					return '🔔';
	...
	return (
		<div className='space-y-4'>
			...
			<div className='flex items-center justify-between text-sm text-soft'>
				<div>
					Показано {отфильтрованныеОбращения.length} из{' '}
					{contacts.length} обращений
				</div>
