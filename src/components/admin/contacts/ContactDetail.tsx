      ...
      <div className="bg-panel border-2 border-neon rounded-lg bevel max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-subtle border border-line rounded-lg bevel p-4">
              ...
              <div className="space-y-3">
                <div>
                  <label className="text-sm text-soft">Имя:</label>
                  <div className="text-base font-medium">{contact.name}</div>
                </div>
                <div>
                  <label className="text-sm text-soft">Email:</label>
                  <div className="text-base font-mono">{contact.email}</div>
                </div>
                {contact.company && (
                  <div>
                    <label className="text-sm text-soft">Компания:</label>
                    <div className="text-base">{contact.company}</div>
                  </div>
                )}
                <div>
                  <label className="text-sm text-soft">Дата обращения:</label>
                  <div className="text-base font-mono">
                    {contact.createdAt.toLocaleDateString("ru-RU")} {contact.createdAt.toLocaleTimeString("ru-RU")}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-subtle border border-line rounded-lg bevel p-4">
              ...
              <div className="space-y-4">
                <div>
                  <label className="text-sm text-soft mb-2 block">Текущий статус:</label>
                  <div className={`inline-flex items-center space-x-2 px-3 py-2 rounded
                ...
                <div>
                  <label className="text-sm text-soft mb-2 block">Приоритет:</label>
                  <div className={`inline-flex items-center space-x-2 px-3 py-2 rounded
                ...
                {/* Изменение статуса */}
                <div>
                  <label className="text-sm text-soft mb-2 block">Изменить статус:</label>
                  <div className="flex items-center space-x-2">
                    <select
                      value={newStatus}
                      onChange={(e) => setNewStatus(e.target.value as ContactData["status"])}
                    ...
                    {newStatus !== contact.status && (
                      <button
                        onClick={обработчикОбновленияСтатуса}
          ...
          <div className="bg-subtle border border-line rounded-lg bevel p-4">
            ...
            <div className="space-y-3">
              <div>
                <label className="text-sm text-soft mb-2 block">Тема:</label>
                <div className="text-lg font-medium text-neon">
                  {contact.subject}
                </div>
              </div>
              <div>
                <label className="text-sm text-soft mb-2 block">Текст обращения:</label>
              ...
              <div className="space-y-3">
                {contact.respondedAt && (
                  <div className="text-sm text-soft">
                    Отправлено: {contact.respondedAt.toLocaleDateString("ru-RU")} {contact.respondedAt.toLocaleTimeString("ru-RU")}
                  </div>
                )}
