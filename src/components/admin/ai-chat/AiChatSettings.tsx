  ...
  return (
    <div className="space-y-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    const provider = e.target.value as "openai" | "groq";
                    обработчикИзменения('provider', provider);
                    // Автоматически меняем модель при смене провайдера
                    if (provider === "groq") {
                      обработчикИзменения('model', 'llama-3.3-70b-versatile');
                      обработчикИзменения('apiKey', process.env.GROQ_API_KEY || '');
                    } else {
                      обработчикИзменения('model', 'gpt-4-turbo');
                ...
                >
                  <option value="groq">Groq (Рекомендуется)</option>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => обработчикИзменения('model', e.target.value)}
                  className="w-full px-4 py-2 bg-panel border border-line rounded text-base
                           focus:border-neon focus:ring-1 focus:ring-neon transition-colors"
                >
                  {настройки.provider === "groq" ? (
                    <>
                      <optgroup label="🔥 Рекомендуемые">
                        <option value="llama-3.3-70b-versatile">Llama 3.3 70B Versatile (Универсальный)</option>
                        <option value="deepseek-r1-distill-llama-70b">DeepSeek R1 Distill 70B (Логическое мышление)</option>
                        <option value="openai/gpt-oss-120b">GPT OSS 120B (Самый мощный)</option>
                        <option value="groq/compound">Groq Compound (Мульти-модель)</option>
                      </optgroup>
                      ...
                      <optgroup label="🌏 Специализированные">
                        <option value="moonshotai/kimi-k2-instruct">Kimi K2 (Китайский/Английский)</option>
                        <option value="moonshotai/kimi-k2-instruct-0905">Kimi K2 0905 (Обновленный)</option>
                        <option value="qwen/qwen3-32b">Qwen3 32B (Alibaba)</option>
                        <option value="allam-2-7b">Allam 2 7B (Арабский)</option>
                      </optgroup>
                      ...
                      <optgroup label="🎙️ Аудио">
                        <option value="whisper-large-v3">Whisper Large v3 (Транскрипция)</option>
                        <option value="whisper-large-v3-turbo">Whisper Large v3 Turbo</option>
                        <option value="playai-tts">PlayAI TTS (Синтез речи)</option>
                    ...
                    <>
                      ...
                      <option value="gpt-4">GPT-4 (Точный)</option>
                      <option value="gpt-4-turbo">GPT-4 Turbo (Рекомендуется)</option>
                    </>
                  )}
                </select>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => обработчикИзменения('apiKey', e.target.value)}
                  className="w-full px-4 py-2 bg-panel border border-line rounded text-base
                           focus:border-neon focus:ring-1 focus:ring-neon transition-colors font-mono text-sm"
                  placeholder={настройки.provider === "groq" ? "gsk_..." : "sk-..."}
                />
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => обработчикИзменения('temperature', parseFloat(e.target.value))}
                  className="w-full"
                />
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => обработчикИзменения('maxTokens', parseInt(e.target.value))}
                  className="w-full px-4 py-2 bg-panel border border-line rounded text-base
                           focus:border-neon focus:ring-1 focus:ring-neon transition-colors"
                />
                <div className="text-xs text-soft mt-1">
          ...
          <div className="bg-subtle border border-line rounded-lg bevel p-6">
            ...
            <div className="space-y-3">
              <div className="flex justify-between p-3 bg-panel rounded border border-line">
                <span className="text-soft">Текущий провайдер:</span>
          ...
          <div className="space-y-4">
            <div>
              ...
              <textarea
                ...
                onChange={(e) => обработчикИзменения('systemPrompt', e.target.value)}
                rows={12}
                className="w-full px-4 py-2 bg-panel border border-line rounded text-base
                         focus:border-neon focus:ring-1 focus:ring-neon transition-colors
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <button
                onClick={() => обработчикИзменения('systemPrompt', 
              ...
              >
                <div className="font-medium text-green-400 mb-1">📝 Краткий стиль</div>
                <div className="text-xs text-soft">Короткие, четкие ответы</div>
              </button>
              ...
              >
                <div className="font-medium text-cyan mb-1">📖 Подробный стиль</div>
                <div className="text-xs text-soft">Развернутые объяснения</div>
              </button>
              ...
              >
                <div className="font-medium text-purple-400 mb-1">💻 Технический стиль</div>
                <div className="text-xs text-soft">Для разработчиков</div>
              </button>
            </div>
          </div>
        </div>
          ...
          <div className="bg-subtle border border-line rounded-lg bevel p-6">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between p-3 bg-panel rounded border border-line">
                <div>
                  <div className="font-medium text-base">Включить модерацию</div>
                  <div className="text-sm text-soft">Проверка содержимого сообщений</div>
                </div>
                <label className="relative inline-flex items-center cursor-pointer">
                  <input
                    type="checkbox"
                    checked={настройки.moderationEnabled}
                    onChange={(e) => обработчикИзменения('moderationEnabled', e.target.checked)}
              ...
              <div className="flex items-center justify-between p-3 bg-panel rounded border border-line">
                <div>
                  <div className="font-medium text-base">Автоответы</div>
                  <div className="text-sm text-soft">AI отвечает автоматически</div>
                </div>
                <label className="relative inline-flex items-center cursor-pointer">
                  <input
                    type="checkbox"
                    checked={настройки.autoRespond}
                    onChange={(e) => обработчикИзменения('autoRespond', e.target.checked)}
                    className="sr-only peer"
                  />
                  <div className="w-11 h-6 bg-gray-600 peer-focus:outline-none rounded-full peer
                               peer-checked:after:translate-x-full peer-checked:after:border-white 
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => обработчикИзменения('responseDelay', parseFloat(e.target.value))}
                  className="w-full"
                />
                <div className="flex justify-between text-xs text-soft mt-1">
            ...
            <div className="space-y-3 max-h-64 overflow-y-auto">
              <div className="p-3 bg-panel rounded border border-line">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-sm font-medium text-base">Заблокировано сообщение</span>
                  <span className="text-xs text-red-400">14:30</span>
                </div>
                <div className="text-xs text-soft">
                  Пользователь: anonymous@user.com • Причина: Неподходящий контент
                </div>
              </div>
              
              <div className="p-3 bg-panel rounded border border-line">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-sm font-medium text-base">Одобрено сообщение</span>
                  <span className="text-xs text-green-400">14:25</span>
                </div>
                <div className="text-xs text-soft">
                  Пользователь: user@example.com • AI ответ отправлен
                </div>
              </div>
              
              <div className="p-3 bg-panel rounded border border-line">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-sm font-medium text-base">Автоматическая проверка</span>
                  <span className="text-xs text-cyan">14:20</span>
                </div>
                <div className="text-xs text-soft">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input
                ...
                onChange={(e) => обработчикИзменения('messageLimit', parseInt(e.target.value))}
                className="w-full px-4 py-2 bg-panel border border-line rounded text-base
                         focus:border-neon focus:ring-1 focus:ring-neon transition-colors"
              />
            </div>
            ...
            <div className="space-y-3">
              <div className="p-3 bg-panel rounded border border-line">
                <div className="flex justify-between items-center">
                  <span className="text-soft">Активных пользователей:</span>
                  <span className="font-mono text-neon">156</span>
                </div>
              </div>
              <div className="p-3 bg-panel rounded border border-line">
                <div className="flex justify-between items-center">
                  <span className="text-soft">Средне сообщений/день:</span>
                  <span className="font-mono text-cyan">18.3</span>
                </div>
              </div>
              <div className="p-3 bg-panel rounded border border-line">
                <div className="flex justify-between items-center">
                  <span className="text-soft">Превышений лимита:</span>
      ...
      <div className="flex items-center justify-between p-4 bg-panel border border-line rounded-lg bevel">
        ...
        >
          {сохранение ? "💾 Сохранение..." : "💾 Сохранить настройки AI"}
        </button>
      </div>
    </div>
  );
