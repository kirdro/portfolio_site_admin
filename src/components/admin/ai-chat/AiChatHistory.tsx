...
export function AiChatHistory() {
  ...
  const отфильтрованныеДиалоги = диалоги.filter(диалог => {
    const статусПодходит = фильтрСтатуса === "all" || диалог.статус === фильтрСтатуса;
    const поискПодходит = поискПользователя === "" || 
      диалог.пользователь.toLowerCase().includes(поискПользователя.toLowerCase()) ||
      диалог.email.toLowerCase().includes(поискПользователя.toLowerCase());
          ...
          <div className="space-y-3">
            <div>
              ...
              <input
                ...
                onChange={(e) => setПоискПользователя(e.target.value)}
            ...
            <div>
              ...
              <select
                value={фильтрСтатуса}
                onChange={(e) => setФильтрСтатуса(e.target.value as any)}
                className="w-full px-3 py-2 bg-panel border border-line rounded text-base
        ...
        <div className="bg-subtle border border-line rounded-lg bevel overflow-hidden">
          <div className="p-4 border-b border-line">
            <div className="flex items-center justify-between">
              <h3 className="font-bold text-base">📜 История диалогов</h3>
              <span className="text-sm text-soft">{отфильтрованныеДиалоги.length} диалогов</span>
            </div>
          </div>
          ...
          <div className="max-h-96 overflow-y-auto">
              ...
              >
                <div className="flex items-start space-x-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                      <div className="font-medium text-base truncate">{диалог.пользователь}</div>
                      <span className={`px-2 py-1 rounded text-xs font-medium ${получитьСтильСтатуса(диалог.статус)}`}>
                        {диалог.статус}
                      </span>
                    </div>
                    <div className="text-sm text-soft truncate">{диалог.email}</div>
                    <div className="text-sm text-soft mt-1 truncate">{диалог.последнееСообщение}</div>
                    <div className="flex items-center justify-between text-xs text-soft mt-2">
                      <span>{диалог.количествоСообщений} сообщений</span>
                      <span>{диалог.последняяАктивность}</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {отфильтрованныеДиалоги.length === 0 && (
              <div className="p-8 text-center text-soft">
                ...
                <div className="text-sm mt-1">Измените фильтры поиска</div>
              </div>
            )}
          </div>
        </div>
        ...
        {выбранныйДиалог ? (
          <div className="space-y-4">
            ...
            <div className="bg-subtle border border-line rounded-lg bevel p-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  <img
                    src={диалоги.find(d => d.id === выбранныйДиалог)?.avatar}
                    alt="User"
                    className="w-12 h-12 rounded border border-line"
                  />
                  <div>
                    <div className="font-bold text-base">
                      {диалоги.find(d => d.id === выбранныйДиалог)?.пользователь}
                    </div>
                    <div className="text-sm text-soft">
                      {диалоги.find(d => d.id === выбранныйДиалог)?.email}
                    </div>
                    <div className="text-xs text-soft">
                      Начат: {диалоги.find(d => d.id === выбранныйДиалог)?.времяСоздания}
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <div className="flex items-center space-x-1">
                    <button
                      onClick={() => обработчикЭкспорта(выбранныйДиалог, "json")}
                    ...
                    <button
                      onClick={() => обработчикЭкспорта(выбранныйДиалог, "txt")}
                    ...
                    <button
                      onClick={() => обработчикЭкспорта(выбранныйДиалог, "csv")}
                  ...
                  <button
                    onClick={() => обработчикБлокировки(выбранныйДиалог)}
            ...
            <div className="bg-subtle border border-line rounded-lg bevel">
              <div className="p-4 border-b border-line">
                <h3 className="font-bold text-base">💬 История сообщений</h3>
              </div>
              ...
              <div className="h-96 overflow-y-auto p-4 space-y-4">
                {(сообщенияДиалога[выбранныйДиалог] || []).map((сообщение) => (
                  ...
                  >
                    ...
                    >
                      <div className="text-sm">{сообщение.содержимое}</div>
                      <div className="text-xs opacity-70 mt-1">{сообщение.времяОтправки}</div>
                    </div>
                  </div>
                ))}
                
                {(сообщенияДиалога[выбранныйДиалог] || []).length === 0 && (
          ...
          <div className="bg-subtle border border-line rounded-lg bevel p-12 text-center">
            ...
            <h3 className="text-xl font-bold text-base mb-2">Выберите диалог</h3>
            <p className="text-soft">Выберите диалог из списка слева для просмотра сообщений</p>
          </div>
        )}
      </div>
    </div>
