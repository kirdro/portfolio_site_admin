...
export function BlogEditor({ 
  ...
  const { isLoading: aiLoading, error: aiError, completeText, improveText, summarizeText, translateText } = useBlogAI();
  ...
  const handleAIAction = async (action: "complete" | "improve" | "summarize" | "translate") => {
    ...
    <div className="blog-editor-container">
      ...
      <style jsx global>{`
        ${Object.entries(editorStyles).map(([selector, styles]) => 
          `${selector} { ${Object.entries(styles as any).map(([prop, value]) => 
      ...
      <div className="relative">
        ...
        {(isLoading || aiLoading) && (
          <div className="absolute inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center">
            <div className="text-neon text-lg font-medium">
              ⏳ {aiLoading ? "Обработка AI запроса..." : "Обработка..."}
            </div>
          </div>
        )}
        ...
        <div className="bg-bg border border-line rounded-lg bevel overflow-hidden">
          <div className="border-b border-line bg-subtle p-3 flex items-center justify-between">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-base font-medium">Редактор статьи</span>
            </div>
          ...
          <div className="min-h-[500px]">
            ...
            <textarea
              ...
              onChange={(e) => {
                ...
                handleContentChange(simpleBlock as PartialBlock[]);
              }}
              defaultValue={
                  ...
                  ? initialContent.map(block => 
                      typeof block === 'object' && block && 'content' in block 
                        ? Array.isArray(block.content) 
                          ? block.content.map(c => typeof c === 'object' && c && 'text' in c ? c.text : '').join('')
        ...
        <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
          <div className="bg-subtle border border-line rounded p-3">
            <div className="font-medium text-neon mb-2">💡 Быстрые команды</div>
            <div className="space-y-1 text-soft">
              <div><kbd className="text-xs bg-panel px-1 rounded">/h1</kbd> - Заголовок 1</div>
              <div><kbd className="text-xs bg-panel px-1 rounded">/h2</kbd> - Заголовок 2</div>
              <div><kbd className="text-xs bg-panel px-1 rounded">/code</kbd> - Блок кода</div>
              <div><kbd className="text-xs bg-panel px-1 rounded">/quote</kbd> - Цитата</div>
            </div>
          </div>
          
          <div className="bg-subtle border border-line rounded p-3">
            <div className="font-medium text-cyan mb-2">⌨️ Горячие клавиши</div>
            <div className="space-y-1 text-soft">
              <div><kbd className="text-xs bg-panel px-1 rounded">Ctrl+B</kbd> - Жирный текст</div>
              <div><kbd className="text-xs bg-panel px-1 rounded">Ctrl+I</kbd> - Курсив</div>
              <div><kbd className="text-xs bg-panel px-1 rounded">Ctrl+K</kbd> - Ссылка</div>
              <div><kbd className="text-xs bg-panel px-1 rounded">Ctrl+Z</kbd> - Отмена</div>
            </div>
          </div>

          {editable && (
            <div className="bg-subtle border border-line rounded p-3">
              <div className="font-medium text-purple-400 mb-2">🤖 AI Ассистент</div>
              <div className="space-y-1 text-soft">
