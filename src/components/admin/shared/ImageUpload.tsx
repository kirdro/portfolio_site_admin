...
export function ImageUpload({
  ...
  // Валидация файла
  const валидироватьФайл = useCallback((file: File): string | null => {
    // Проверка формата
    if (!acceptedFormats.includes(file.type)) {
      return `Поддерживаемые форматы: ${acceptedFormats.map(f => f.split('/')[1]).join(', ').toUpperCase()}`;
    }
  ...
  // Обработка файлов
  const обработатьФайлы = useCallback(async (files: FileList | null) => {
  ...
  const обработчикDrop = useCallback((e: React.DragEvent) => {
    e.preventDefault();
    setIsDragging(false);
  ...
  const обработчикВыбораФайла = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  return (
    <div className="space-y-4">
      ...
      >
        ...
        {isUploading ? (
          ...
          <div className="space-y-3">
            ...
            <div className="text-sm text-neon">Загрузка изображения...</div>
            ...
            <div className="text-xs text-soft">{uploadProgress}%</div>
          </div>
        ) : currentImageUrl ? (
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-1">
              <div className="text-base font-medium">
                {isDragging ? "Отпустите файл для загрузки" : "Загрузить изображение"}
              </div>
      ...
      <div className="text-xs text-soft text-center space-y-1">
        <div>
          Поддерживаемые форматы: {acceptedFormats.map(f => f.split('/')[1]).join(', ').toUpperCase()}
        </div>
        <div>
