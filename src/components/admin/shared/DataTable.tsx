...
export function DataTable<T>({
  ...
  const обработчикСортировки = useCallback((key: string) => {
  ...
  const обработчикКликаСтроки = useCallback((item: T, index: number) => {
              ...
              {Array.from({ length: 5 }, (_, index) => (
                <tr key={index} className="border-t border-line">
                  {columns.map((column) => (
                    ...
                    >
                      <div className="h-4 bg-subtle rounded-md animate-pulse"></div>
                    </td>
                  ))}
                </tr>
              ))}
  ...
  if (!отсортированныеДанные.length) {
    return (
      <div className="bg-panel border border-line rounded-lg bevel p-8 text-center">
        ...
        <div className="text-soft/60 text-sm mt-2">Попробуйте изменить фильтры поиска</div>
      </div>
    );
  }

  return (
    <div className={`bg-panel border border-line rounded-lg bevel ${className}`}>
      <div className="overflow-x-auto">
        <table className="min-w-full">
          <thead className="bg-subtle">
            <tr>
              {columns.map((column) => (
                <th
                  key={column.key}
                  onClick={() => обработчикСортировки(column.key)}
                  className={`
          ...
          <tbody className="divide-y divide-line">
            {отсортированныеДанные.map((item, index) => (
              <tr
                key={index}
                onClick={() => обработчикКликаСтроки(item, index)}
                className={`
